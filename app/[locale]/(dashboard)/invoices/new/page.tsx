...
export default function CreateInvoicePage() {
  ...
  const updateItem = (index: number, field: keyof LineItem, value: string | number | null) => {
    ...
    // If product selected, fill in description & price
  ...
  const onSubmit = async (data: CreateInvoiceInput) => {
    try {
      ...
      toast.success(t('messages.createSuccess', { entity: t('invoices.title') }));
  ...
  const handleFormSubmit = () => {
    // Sync items into form before validation
      ...
      <Form {...form}>
        <form onSubmit={(e) => { e.preventDefault(); handleFormSubmit(); }} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3">
            <Button type="button" variant="outline" onClick={() => router.push(`/${locale}/invoices`)}>
              {t('common.cancel')}
            </Button>
