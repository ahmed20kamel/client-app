...
const LITBEAM_PRODUCTS = ['LM', 'LINEAR METER'];
...
export default function CreateQuotationPage() {
  ...
  const updateItem = (index: number, field: keyof LineItem, value: string | number | null) => {
    setItems((prev) => {
      ...
      // Recalculate linear meters if qty or length changes
                ...
                <CardContent>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="flex items-center gap-1.5">
                            ...
                            {t('quotations.deliveryCharges')} (AED)
                          </FormLabel>
                          <FormControl>
                  ...
                  <div className="mt-6 border-t pt-4 space-y-2 max-w-sm ms-auto">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">{formatCurrency(calculations.subtotal)} AED</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">{t('quotations.tax')} ({taxPercent}%)</span>
                      <span className="font-medium">+{formatCurrency(calculations.taxAmount)} AED</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">{t('quotations.deliveryCharges')}</span>
                      <span className="font-medium">+{formatCurrency(deliveryCharges)} AED</span>
                    </div>
                    <div className="flex justify-between text-base font-bold border-t pt-2">
                      <span>{t('quotations.total')}</span>
                      <span>{formatCurrency(calculations.total)} AED</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
