...
import {
  ...
  Phone, Building2, Truck, FileText, Package, Printer, Package2,
} from 'lucide-react';
...
export default function TaxInvoiceDetailPage() {
  ...
  const handleCreateDN = () => {
    if (!invoice) return;
    router.push(`/${locale}/delivery-notes/new?taxInvoiceId=${invoice.id}`);
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-6 py-5 border-b border-border">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(invoice.deliveryNotes?.length ?? 0) === 0 && (
              <Button size="sm" className="btn-premium" onClick={handleCreateDN}>
          ...
          <Card className="shadow-premium">
            <CardContent className="pt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                <InfoRow icon={User} label={t('quotations.engineerName')} value={invoice.engineerName} />
                <InfoRow icon={Phone} label={t('quotations.mobileNumber')} value={invoice.mobileNumber} />
