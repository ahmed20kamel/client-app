...
import { createPurchaseOrderSchema, CreatePurchaseOrderInput } from '@/lib/validations/purchase-order';
...
export default function CreatePurchaseOrderPage() {
  ...
  const updateItem = (index: number, field: string, value: string | number | null) => {
    ...
    // Auto-fill price from product if selected
  ...
  const onSubmit = async (data: CreatePurchaseOrderInput) => {
    ...
    try {
      ...
      toast.success(t('messages.createSuccess', { entity: t('purchaseOrders.title') }));
  ...
  // Sync items to form before submit
      ...
      <Form {...form}>
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3">
            <Button type="button" variant="outline" onClick={() => router.push(`/${locale}/purchase-orders`)}>
              {t('common.cancel')}
            </Button>
