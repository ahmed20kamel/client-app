...
export default function MonthlyPayrollPage() {
  ...
  const setField = (empId: string, field: string, value: string) => {
    setRows(prev => prev.map(r => {
      ...
      u.totalPayment = u.grossSalary;
      if (u.employee.paymentMethod === 'Cash') { u.cashAmount = u.grossSalary; u.wpsAmount = 0; }
      else { u.wpsAmount = Math.max(0, u.basicSalary - u.absentDeduction); u.cashAmount = Math.max(0, u.grossSalary - u.wpsAmount); }
      return u;
    }));
  };
  ...
  const exportCSV = () => {
    const headers = ['Code','Name','Cost Center','WPS Entity','Basic','Allowances','Other Allow','Total Salary','Work Days','Absent','OT Hrs','OT Amt','Absent Deduct','Allow Adj','Deduction','Adj','Gross','WPS Amt','Cash','OT Pay','Total'];
        ...
        {rows.length > 0 && (
          <div className="flex items-stretch divide-x divide-border border-b-2 border-border">
            {[
              ...
              { label: 'Deductions',    value: sum(rows, 'absentDeduction'),  cls: 'text-red-600' },
          ...
          {Object.entries(grouped).map(([cc, ccRows]) => (
            <div key={cc} className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-[12px] border-collapse" style={{ minWidth: '1380px' }}>
                  <thead>
